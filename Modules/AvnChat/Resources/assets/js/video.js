                    ...
                    success: function () {
                        ...
                        janus.attach({
                            ...
                            success: function (pluginHandle) {
                                ...
                                sfutest.send({ message: join_video_room });
                            },
                            ...
                            onmessage: function (msg, jsep) {
                                    ...
                                    if (event === "joined") {
                                        ...
                                        if (msg["publishers"]) {
                                            ...
                                            for (let f in list) {
                                                ...
                                                let slot = feedStreams[id]
                                                    ? feedStreams[id].slot
                                                    : null;
                                                let remoteVideos = feedStreams[
                                                    id
                                                ]
                                                    ? feedStreams[id]
                                                          .remoteVideos
                                ...
                                if (jsep) {
                                    ...
                                    if (
                                        ...
                                        $(".myvideo").hide();
                                        if (
                                            $(".no-video-container").length ===
                                            0
                                        ) {
                                            $(
                                                "#start-call-modal .modal-body, #in-call-modal .modal-body "
                                            ).append(
                                                '<div class="no-video-container">' +
                                                    '<img src="' +
                                                    window.user.img +
                                                    '" class="avatar-xl rounded-circle" style="object-fit: cover;">' +
                                                    '<p class="no-video-text">' +
                                                    window.user.name +
                                                    "</p>" +
                                                    "</div>"
                                            );
                            ...
                            onlocaltrack: function (track, on) {
                                ...
                                if (track.kind === "audio") {
                                    ...
                                    $(
                                        ...
                                        '<video class="rounded centered myvideo' +
                                            trackId +
            ...
            if (!on) {
                ...
                if (track.kind === "video" && feed) {
                    ...
                    if (feed.remoteVideos === 0) {
                        // No video, at least for now: show a placeholder
                        if (
                            $("#videoremote" + slot + " .no-video-container")
                                .length === 0
                        ) {
            ...
            if (track.kind === "audio") {
                ...
                if (!bitrateTimer[slot]) {
                    ...
                    bitrateTimer[slot] = setInterval(function () {
                        ...
                        let width = $("#videoremote" + slot + " video").get(
                            0
                        ).videoWidth;
                        let height = $("#videoremote" + slot + " video").get(
                            0
                        ).videoHeight;
                        if (width > 0 && height > 0) {
                            let res = width + "x" + height;
                            if (simulcastStarted[slot]) res += " (simulcast)";
                            else if (svcStarted[slot]) res += " (SVC)";
                            $("#curres" + slot)
                                .removeClass("hide")
                                .text(res)
                                .show();
...
function updateSimulcastSvcButtons(feed, substream, temporal) {
    ...
    if (temporal === 0) {
        toastr.success(
            "Capped " + what + " temporal layer! (lowest FPS)",
            null,
            { timeOut: 2000 }
        );
        $("#tl" + index + "-2")
        ...
        toastr.success(
            "Capped " + what + " temporal layer! (medium FPS)",
            null,
            { timeOut: 2000 }
        );
        $("#tl" + index + "-2")
        ...
        toastr.success(
            "Capped " + what + " temporal layer! (highest FPS)",
            null,
            { timeOut: 2000 }
        );
        $("#tl" + index + "-2")
