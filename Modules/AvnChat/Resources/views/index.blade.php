...
@section('content')
    <div class="container">
        <div class="row mt-3">
            <!-- start chat users-->
            <div class="col-xl-3 room-col" id="room-container">
                @include('avnchat::components.chat-users', [$user, $rooms])
            </div>
            <!-- end chat users-->
            <!-- chat area -->
            <div class="col-xl-6 chat-col d-none" id="chat-container">
                <div class="card shadow-lg mb-0 d-lg-none chat-navigation rounded-0"
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        window.user = {
            ...
            name: "{{ $user->name }}",
            type: "{{ $user->type }}",
            img: "{{ $user->profile->img ?? 'resources/assets/images/users/avatar-1.jpg' }}",
        };
        ...
        $(document).ready(function() {
            ...
            window.Echo.private('joined.user.{{ $user->id }}')
                .listen('.newRoom', (e) => {
                    window.room_container.newRoom(e.room_id)
                })
            ...
            function Echo_listen(chanel_name) {
                window.Echo.private(chanel_name)
                    .listen('.newMessage', (e) => {
                        if (e.message.user_id != {{ $user->id }}) {
                            if (room_id == e.message.room_id) {
                                window.chat_container.addReceiveMessage(e.message)
                            }
                            window.room_container.getRoom(e.message.room_id).addNewMessage(e.message)
                            window.chat_container.scrollBottom();
                        }
                    })
                    .listen('.newSystemMessage', (e) => {
                    ...
                    .listen('.newUser', (e) => {
                        $.ajax({
                            ...
                            success: function(res) {
                                var room = window.room_container.getRoom(res.room_id);
                                room.updateRoomName(res.name);
                                room.updateImg(res.imgs);
                                room.updateRoomStatus(
                                    is_workspace = res.is_workspace,
                                    has_session = res.has_session,
                                    has_customers = res.customers
                                );
                    ...
                    .listen('.startSession', (e) => {
                        ...
                        window.room_container.getRoom(e.message.room_id).updateRoomStatus(
                            is_workspace = true,
                            has_session = true,
                            has_customers = true
                        );
                    ...
                    .listen('.endSession', (e) => {
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                window.room_container.getRoom(res.room_id).updateRoomStatus(
                                    is_workspace = res.is_workspace,
                                    has_session = res.has_session,
                                    has_customers = res.customers
                                );
                    ...
                    .listen('.newCall', (e) => {
                        if (!is_calling && !is_incall && !is_ringing) {
                            window.stop_call_modal.hide()
                            $('.microphone').attr('disabled', '')
                            $('.stop-call').attr('disabled', '')
                            is_ringing = true
            ...
            $('.chat-rooms-conatiner').on('click', '.chat-room', function() {
                ...
                if (room_id != $(this).data('id')) {
                    ...
                    clearInterval(count_up);
                    window.chat_container.clearMessage()
                    preview_images = []
                    images = []
                    ...
                    load_message = $.ajax({
                        ...
                        success: function(res) {
                            if (res.length) {
                                window.chat_container.last_load = res[res.length - 1].id
                            } else {
            ...
            $(".update-chat-room").on('click', function(param) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        var room = window.room_container.getRoom(res.id);
                        room.updateRoomName(res.name);
                        room.updateImg([res.img]);
                        room.updateRoomStatus(
                            is_workspace = res.is_workspace,
                            has_session = res.has_session,
                            has_customers = res.customers
            ...
            function set_name_image_chat_info(name, imgs) {
                ...
                if (imgs.length == 1) {
                    $('.chat-info .chat-info-img').attr('src', imgs[0] ??
                        'resources/assets/images/users/avatar-1.jpg')
                    $('.chat-info .chat-info-img').attr('alt', name)
                    $('.chat-info .chat-info-img').removeClass('d-none')
                    $('.chat-info .chat-info-imgs').addClass('d-none')
                } else {
                    var htm = ''
                    ...
                    $('.chat-info .chat-info-imgs').html(htm)
                    $('.chat-info .chat-info-imgs').removeClass('d-none')
                    $('.chat-info .chat-info-img').addClass('d-none')
                }
            }
                    ...
                    success: function(res) {
                        var new_message_in_this_room = res.filter(function(message) {
                            if (!$('.chat-room[data-id=' + message.room_id + ']')
                                .length) {
                                window.room_container.newRoom(message.room_id)
                            } else {
                                ...
                                if (message.user_id) {
                                    window.room_container.getRoom(message.room_id)
                        ...
                        window.chat_container.addListMessages(new_message_in_this_room);
                        window.chat_container.scrollBottom();
                    },
                    error: function() {}
                });
        ...
        $(document).ready(function() {
            $('#start-call').on('click', function() {
                ...
                if ($('.chat-info .chat-info-imgs').hasClass('d-none')) {
                    ...
                    $('#in-call-modal .chat-info-imgs').html($('.chat-info .chat-info-imgs').html())
                    $('#in-call-modal .chat-info-imgs').removeClass('d-none')
                    $('#in-call-modal .chat-info-img').addClass('d-none')
                }
                $('#in-call-modal .joined-users').html('')
                $('#stop-call-modal .chat-info-name').html($('.chat-info .chat-info-name').html())
                if ($('.chat-info .chat-info-imgs').hasClass('d-none')) {
                    $('#stop-call-modal .chat-info-img').attr('src', $('.chat-info .chat-info-img').attr(
                        'src'));
                    $('#stop-call-modal .chat-info-img').removeClass('d-none')
                    $('#stop-call-modal .chat-info-imgs').addClass('d-none')
                } else {
                    $('#stop-call-modal .chat-info-imgs').html($('.chat-info .chat-info-imgs').html())
                    $('#stop-call-modal .chat-info-imgs').removeClass('d-none')
                    $('#stop-call-modal .chat-info-img').addClass('d-none')
                }
                $('#start-call').addClass('d-none')
            ...
            $('.stop-call').on('click', function() {
                ...
                window.is_ringing = false
                window.is_incall = false
                $('#mini-in-call, #mini-start-call').addClass('d-none')
                leave_audio_room()
            });
    ...
    <script src="{{ asset('Modules/AvnChat/Resources/assets/js/video.js') }}"></script>
@endsection
@section('css')
    <style>
        .room-name,
